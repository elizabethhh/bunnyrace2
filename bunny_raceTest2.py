...
class Bunny(): 
    ...
    # GAME METHODS
    print("Hello person. This is a game purely based on random functions. Don't be upset if you lose since you have a low chance of winning. This is meant to teach you a lesson about life. Good luck.")
    bet = input("Which bunny do you think will win? Choose a bunny from numbers 1-7. ")
    

    if bet >= str(1) and bet <= str(7):
        test = False
    else:
        input("Please enter a number from 1-7. ")
        
        
    def race(self, screen, colors, screen_width, screen_height):

        ball_color = random.choice(colors)
        if self.x_location >= screen_width - self.ball_size * 2.5:
            self.x_speed = self.x_speed * 0
            
        self.x_location += self.x_speed 

        img = pygame.image.load("C:/Users/Elizabeth Song/Downloads/bunny10.png")
        screen.blit(img, (self.x_location, self.y_location))
...
if answer == 1:
	print("Congratulations! You guessed the right number! That bunny number was the fastest. Be a humble chicken.")
else:
	print("Sorry! The other bunny beat the bunny you guessed. Nice try and have a good day! If at first you don't succeed, try, try again. ")
print("Inspired by Aesop's Fable.")
